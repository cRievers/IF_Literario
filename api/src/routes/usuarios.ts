...
// GET /api/usuarios — lista avaliadores e orientadores (excluindo deletados)
router.get('/', requireAuth, requireRole(['ADMIN']), async (_req: AuthRequest, res: Response, next: NextFunction): Promise<any> => {
  try {
    const usuarios = await prisma.user.findMany({
      where: {
        deletedAt: null,
        role: { in: ROLES_GERENCIAVEIS },
      },
